...
function MilestoneDetails({
  ...
  useEffect(() => {
    if (milestoneDetail.status) {
      console.log("milestoneDetail.status", milestoneDetail.status);
      setStatusStyle(getStatusColor(milestoneDetail.status));
    }
  ...
  return (
    <div className="max-w-4xl p-4 space-y-4 rounded-lg sm:p-6 dark:bg-gray-500">
      <div>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-base font-medium sm:text-xl dark:text-gray-150 truncate w-48 sm:w-full">
                {milestoneDetail.prototype}
              </p>
              <p className="text-xs font-medium dark:text-gray-200">
                {milestoneDetail.milestoneRange + " " + "milestones"}
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-col w-full">
        ...
        <div className="flex items-center justify-between p-2">
          <p className="text-sm font-medium text-gray-500 dark:text-gray-150">
            {milestoneDetail.amount}
            {" " + milestoneDetail.paidIn}
          </p>
          <p className="text-sm font-medium text-gray-500 dark:text-gray-150">
            {milestoneDetail.submittedOn}
          </p>
        </div>
      </div>
      ...
      {/* Rejection Reason */}
      {milestoneDetail.status.toLowerCase() === "rejected" && (
        <div className="flex flex-col w-full">
          <div className="flex items-center justify-between px-2 py-1 bg-gray-100 dark:bg-gray-600">
            <p className="text-xs font-medium text-gray-400 dark:text-gray-200">
          ...
          <div className="flex items-center justify-between p-2">
            <p className="text-sm font-medium text-gray-500 dark:text-gray-150">
              {milestoneDetail.rejectionReason}
            </p>
          </div>
        </div>
      )}
