...
const MakeInvoicePayment = ({ handlePayment }: MakeInvoicePaymentProps) => {
  ...
  const goToDashboard = () => (window.location.href = RoutePaths.DASHBOARD);
  ...
  return (
    <form className="w-full h-fit space-y-8" onSubmit={handleSubmit}>
      ...
      <div className="flex items-center w-full gap-2 flex-col md:flex-row">
        ...
        <button className="flex items-center justify-center w-full gap-1 px-4 py-2 text-sm font-medium text-white transition duration-150 ease-in-out border-0 rounded-full outline-none cursor-pointer h-14 bg-primary-200">
          {buttonText}
        </button>
      </div>
    </form>
