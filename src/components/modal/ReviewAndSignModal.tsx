...
import { CheckMarkIcon } from "../../assets/svg/svg";
import useModal from "../../hooks/useModal";
import ContractReviewModal from "./ContractReviewModal";
import ErrorMessage from "../form/ErrorMessage";
import ContractCompletionModal from "./ContractCompletionModal";
type FormProps = {
  legalName: string;
...
function ReviewAndSignModal() {
  ...
  const onSubmit = async (data: FormProps) => {
    ...
    const contractCreated = await new Promise((resolve) =>
      setTimeout(() => resolve(true), 2000)
    );

    console.log("Contract signed!", contractCreated);
    reset();
    ...
    <div className="relative flex flex-col space-y-8 h-dvh sm:h-fit">
      <div className="flex flex-col items-center pb-4 space-y-2 ">
        <div className="flex flex-row gap-2.5 sm:flex-col sm:gap-8  w-full">
          ...
          <p className="w-full text-xl font-semibold text-center text-gray-500 sm:text-3xl dark:text-gray-150">
            Review & Sign
          </p>
        </div>
        <p className="max-w-xs text-xs font-medium text-center text-gray-400 text-pretty dark:text-gray-200">
      ...
      >
        <div className="space-y-8">
          <div className="form-control">
            ...
            />
            <ErrorMessage
              isVisible={errors.legalName ? true : false}
          ...
          <div className="flex items-start gap-2">
            <label className="relative">
              <input
                ...
                className="transition-colors duration-300 ease-in-out rounded outline-none appearance-none checked:border-transparent not-checked:border-2 peer size-5 checked:bg-primary-200 not-checked:border-gray-150"
            ...
            <p className="text-xs font-medium text-gray-400 dark:text-gray-200">
              ...
              <a
                href="#"
                className="font-semibold dark:text-primary-400 text-primary-200"
              >
                {" "}
                Terms & Conditions
              </a>{" "}
        ...
        <div className="flex flex-col w-full gap-2 xs:flex-row">
          <button
            className="w-full border border-gray-500 rounded-full cursor-pointer h-14 dark:border-gray-100 hover:opacity-90"
            onClick={OpenContractReviewModal}
          >
            Back
          </button>
          ...
          <button
            type="submit"
            className="w-full p-2 font-medium text-white rounded-full cursor-pointer h-14 bg-primary-200 hover:opacity-90"
          >
